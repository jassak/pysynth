...
class Granular:
    ...
    ::
        ...
        pos = Oscillator("triangle").at(0.1).render(4.0) * 0.5 + 0.5
        sig = Granular(sample, position=pos, grain_size=0.06, density=25).at().render(4.0)
    """
    ...
    def at(self, hz: float | Signal | None = None) -> Generator:
        """Return a :class:`Generator` for rendering granular output.
        ...
        if hz is not None:
            if sample.root_pitch is None:
                raise ValueError(
                    "sample.root_pitch must be set to use .at(hz) with a frequency. "
                    "Use .at() for default pitch, or set root_pitch on the sample."
                )
        ...
        def render(dur: float, sr: int = SAMPLE_RATE) -> Signal:
            ...
            for onset_idx in onsets:
                ...
                # Clamp to valid range (np.interp handles out-of-bounds via left/right)
        ...
        name = f"Granular.at({hz})" if hz is not None else "Granular.at()"
