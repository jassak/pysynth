...
def _svf_lowpass(x, cutoff_arr, sr, n_stages):
    n = len(x)
    ic1 = np.zeros(n_stages)
    ic2 = np.zeros(n_stages)
    out = np.empty(n)
    pi = np.pi
    k = 2.0  # Butterworth Q per stage
    for i in range(n):
        fc = cutoff_arr[i]
        if fc < 1.0:
            fc = 1.0
...
@numba.njit(cache=True)
def _svf_highpass(x, cutoff_arr, sr, n_stages):
    n = len(x)
    ic1 = np.zeros(n_stages)
    ic2 = np.zeros(n_stages)
    out = np.empty(n)
...
@numba.njit(cache=True)
def _svf_bandpass(x, low_arr, high_arr, sr, n_stages):
...
def _svf_filter_mono(x, order, btype, cutoff_arr, sr):
    """Apply SVF filter to a mono signal."""
    n_stages = max(1, order // 2)
    x64 = x.astype(np.float64)
    c64 = cutoff_arr.astype(np.float64)
    if btype == "low":
        return _svf_lowpass(x64, c64, sr, n_stages).astype(np.float32)
    else:
        return _svf_highpass(x64, c64, sr, n_stages).astype(np.float32)


def _svf_filter(data, order, btype, cutoff, nyquist):
    ...
    if data.ndim == 2:
        ...
        for c in range(data.shape[1]):
            out[:, c] = _svf_filter_mono(data[:, c], order, btype, cutoff_arr, sr)
        return out
    return _svf_filter_mono(data, order, btype, cutoff_arr, sr)


def _svf_bandpass_filter(data, order, low, high, nyquist):
    ...
    if data.ndim == 2:
        ...
        for c in range(data.shape[1]):
            ...
            out[:, c] = _svf_bandpass(x64, low_arr, high_arr, sr, n_stages).astype(np.float32)
        return out
    x64 = data.astype(np.float64)
    return _svf_bandpass(x64, low_arr, high_arr, sr, n_stages).astype(np.float32)
...
class LowPassFilter(Effect):
    ...
    def __call__(self, sig: Signal) -> Signal:
        ...
        if isinstance(self.cutoff_hz, Signal):
            return Signal(
                _svf_filter(sig.data, self.order, "low", self.cutoff_hz, nyquist),
                sig.sample_rate,
            )
...
class HighPassFilter(Effect):
    ...
    def __call__(self, sig: Signal) -> Signal:
        ...
        if isinstance(self.cutoff_hz, Signal):
            return Signal(
                _svf_filter(sig.data, self.order, "high", self.cutoff_hz, nyquist),
                sig.sample_rate,
            )
...
class BandPassFilter(Effect):
    ...
    def __call__(self, sig: Signal) -> Signal:
        ...
        if isinstance(self.low_hz, Signal) or isinstance(self.high_hz, Signal):
            return Signal(
                _svf_bandpass_filter(sig.data, self.order, self.low_hz, self.high_hz, nyquist),
                sig.sample_rate,
            )
